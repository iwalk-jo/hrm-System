...
class Loan_model extends CI_Model {
	...
	{
			    return $this->db->select('count(DISTINCT(ln.loan_id)) as loan_id,ln.*,p.employee_id,p.first_name,p.last_name')   
            ->from('grand_loan ln')
...
public function installment_view()
	{
			 ...
			 return $this->db->select('count(DISTINCT(ins.loan_inst_id)) as loan_inst_id,ins.*,p.employee_id,p.first_name,p.last_name')   
...
public function report_loan($id){
	$this->db->select('a.employee_id,a.pos_id,a.first_name,a.last_name,a.picture,b.date_of_approve,b.amount,b.installment,b.loan_id,b.repayment_amount,sum(c.payment) as payment');    
$this->db->from('employee_history a');
$this->db->join('grand_loan b', 'a.employee_id = b.employee_id', 'inner');
$this->db->join('loan_installment c', 'a.employee_id = c.employee_id', 'inner');
$this->db->where('b.employee_id', $id );
$this->db->group_by('b.date_of_approve');
$query = $this->db->get();
$result = $query->result();
